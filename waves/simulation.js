...
var Simulator = function (canvas, width, height) {
    ...
    for (var zIndex = 0; zIndex < GEOMETRY_RESOLUTION; zIndex += 1) {
        for (var xIndex = 0; xIndex < GEOMETRY_RESOLUTION; xIndex += 1) {
            ...
            var u = xIndex / (GEOMETRY_RESOLUTION - 1);
            var v = zIndex / (GEOMETRY_RESOLUTION - 1);

            oceanData.push((u * RESOLUTION + 0.5) / RESOLUTION);
            oceanData.push((v * RESOLUTION + 0.5) / RESOLUTION);
        }
    }
    ...
    var initialSpectrumTexture = buildTexture(gl, INITIAL_SPECTRUM_UNIT, gl.RGBA, gl.FLOAT, RESOLUTION, RESOLUTION, null, gl.REPEAT, gl.REPEAT, gl.NEAREST, gl.NEAREST),
        ...
        normalMap = buildTexture(gl, NORMAL_MAP_UNIT, gl.RGBA, gl.FLOAT, RESOLUTION, RESOLUTION, null, gl.REPEAT, gl.REPEAT, gl.LINEAR_MIPMAP_LINEAR, gl.LINEAR),
    ...
    this.render = function (deltaTime, projectionMatrix, viewMatrix, cameraPosition) {
        ...
        gl.activeTexture(gl.TEXTURE0 + NORMAL_MAP_UNIT);
        ...
        for (var zOffset = -TILES; zOffset <= TILES; ++zOffset) {
            for (var xOffset = -TILES; xOffset <= TILES; ++xOffset) {
                gl.uniform3f(oceanProgram.uniformLocations['u_offset'], GEOMETRY_SIZE * xOffset, 0.0, GEOMETRY_SIZE * zOffset);
